...
public class SearchingBasedOnFiles extends SharedTest {
    ...
    public void testFiles() throws URISyntaxException, IOException {
        ...
        Path searchFolderPath = Paths.get(searchFolder.toURI());

        List<File> tests = Arrays.asList(searchFolderPath.toFile().listFiles());
        ...
        for (Input input : inputs) {
            for (Map.Entry<Integer, List<String>> givenEntry : input.getGivenData().entrySet()) {
                ...
                for (String articleLine : articleLines) {
                    ...
                    List<Double> aValues = Arrays.asList(articleLine.split(",")).stream().map(Double::valueOf).collect(Collectors.toList());
                    Article article = articleFactory.create(id, aValues.get(0), aValues.get(1), aValues.get(2));
                    articleRepository.saveReceived(article);
                }


                System.out.println("Processing " + n);
                List<Double> givenValues = Arrays.asList(givenEntry.getValue().get(0).split(",")).stream().map(Double::valueOf).collect(Collectors.toList());

                Gap maxGap = gap(givenValues.get(0), givenValues.get(1), givenValues.get(2));
                Gap requiredGap = gap(givenValues.get(0), givenValues.get(3), givenValues.get(4));
    ...
    private void assertBestFitBiggerThenReq(String s, SearchResult searchResult) {
        if (s.contains("notFound")) {
            ...
            List<Double> thenValues = Arrays.asList(s.split(",")).stream().map(Double::valueOf).collect(Collectors.toList());
    ...
    private void addData(Map<Integer, List<String>> data, Path given) {
        Stream.of(given.toFile().listFiles()).forEach(f -> {
            try {
